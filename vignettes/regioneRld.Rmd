...
time. To compare z-scores coming from multiple analysis, different strategies have been introduced to normalize the z-score and to improve the 
p-value calculations. Taken together, regioneRld aims to be a novel and precious addition to NGS tools and for whole genome analysis.
...
regioneRld can use all the evaluation strategies present in
`r BiocStyle::Biocpkg("regioneR")`.

### Randomization functions

regioneRld can use all the randomization strategies present in
`r BiocStyle::Biocpkg("regioneR")`.
...
plotCrosswiseMatrix(cw_Alien_RaR,matrix_type = "crosswise",ord_mat = ord)
plotCrosswiseMatrix(cw_Alien_ReG,matrix_type = "crosswise" ,ord_mat = ord)
plotCrosswiseMatrix(cw_Alien_cRaR,matrix_type = "crosswise",ord_mat = ord)
plotCrosswiseMatrix(cw_Alien_ReR,matrix_type = "crosswise",ord_mat = ord)

plotCrosswiseMatrix(cw_Alien_RaR,matrix_type = "correlation",ord_mat = ord)
plotCrosswiseMatrix(cw_Alien_ReG,matrix_type =  "correlation" ,ord_mat = ord)
plotCrosswiseMatrix(cw_Alien_cRaR,matrix_type =  "correlation",ord_mat = ord)
plotCrosswiseMatrix(cw_Alien_ReR,matrix_type =  "correlation",ord_mat = ord)
...
plotSinglePT(cw_Alien_ReG, RS1 = "regA", RS2 = "reg_no_A")
...
To test how perform  `r BiocStyle::Biocpkg("regioneRld")` in a real scenario we download all transcrption factors present in the ENCODE (ref) project relative to HepG2 (261 ChIPseq experimets from human liver cancer). The complete GRList is possible to download at ("....").
