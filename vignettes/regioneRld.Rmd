...
analysis means the calculation of permutation tests between all possible 
combinations of a set of `Alist` regions with a second set of `Blist` regions. 
The result of `crosswisePermTest()` is an S4 object of class `genoMatriXeR` 
that contains three slots: *parameters*, *multiOverlaps* and *matrix*.
...
we need to use the fixed order of clusterization for all matrices. 
