...
In the world of genetic and epigenetic analysis, a huge amount of data is created every day, and many of these results can be summarized as region sets. Sets of regions are a set of genomic regions defined by coordinates in the genome, associated with metadata (values characterizing the genomic region). Some of the most popular techniques such as ChIPseq, Cut&Run, HI-C (among others) produce this type of result, but not only: with region sets one can identify regions of interest in the genome itself, such as repeated regions like SINEs LINEs, STRs or active regions like promoters, enhancers etc. Meaningful interpretation of the association between sets of regions is one of the biggest challenges when it comes to giving global meaning to the data created by a biological experiment by comparing it with existing database data. To address this issue, in 2015 we published `r BiocStyle::Biocpkg("regioneR")`, an R package we developed for statistical evaluation of the association between sets of genomic regions. The main limitation of regioneR is that it can only perform association analysis between two region sets at a time, the association value used "z-score" cannot be directly compared with that of other association sets because it is influenced by the nature of the region sets, and a further problem arises with respect to the calculation time when the number of region sets to be analyzed is high. here, we now present `r BiocStyle::Rpackage("regioneReloaded")`, an R package that is the natural evolution of `r BiocStyle::Biocpkg("regioneR")` and allows to calculate the statistical association between multiple regions sets at the same time. `r BiocStyle::Rpackage("regionReloaded")` comprises a set of functions created to analyze clusters of associations and to facilitate the interpretation of results including strategies to improve p-value calculations and normalized z-scores coming from multiple analysis to allow for their direct comparison. `r BiocStyle::Rpackage("regioneReloaded")` also builds upon `r BiocStyle::Biocpkg("regioneR")` by adding new plotting functions for obtaining publication-ready graphs. Taken together, it aims to be a novel and valuable addition to the repertoire of available tools for the analysis of high-content genomic datasets.
