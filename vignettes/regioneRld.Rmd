...
<!-- Explain what exactly the nZS corrects? The dependency of the ZS value on the number of regions in the
region sets tested? -->
...
the following code, the permutation process is computing intensive and its calculation time depends on the 
number of cores called by the parameter \code{\link{mc.cores}} (see `r BiocStyle::Biocpkg("regioneR")`).
The result of the permutation test obtained by running the code below is included as a precomputed example dataset
